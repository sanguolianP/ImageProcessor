...
vector<complex<double> > matrix::getRow(int r) const {
    ...
    if(!checkRange(r,0)) throw range_error("in getRow,out of range!");
    for(size_t i=0;i<cols_;i++)
...
vector<complex<double> > matrix::getCol(int c) const {
    vector<complex<double> >ret;
    if(!checkRange(0,c)) throw range_error("in getCol,out of range!");
    for(size_t i=0;i<rows_;i++)
...
complex<double>& matrix::getElem(int r, int c) const{
    ...
    if(t_==0)
        return data_[(r+startRow_)%rows_][(c+startCol_)%cols_];
    else
        return data_[(c+startCol_)%cols_][(r+startRow_)%rows_];
}
